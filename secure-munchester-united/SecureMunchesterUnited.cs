...
public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support) => support switch {
        var security when support is Security && !(support is SecurityJunior) && !(support is SecurityIntern) && !(support is PoliceLiaison) => $"{security.Title} Priority Personnel",
        var staff when support is Staff => staff.Title,
